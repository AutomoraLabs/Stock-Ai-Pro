...
class PromptCardWidget(QFrame):
    ...
    def delete_prompt(self):
        reply = QMessageBox.question(self, 'Confirm Delete', "Delete this prompt?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
    ...
    def export_all_prompts(self):
        ...
        for idx, p in enumerate(self.prompts_list):
            ...
            if custom_title:
                ...
                seo_match = re.search(r'(?:SEO[- ]?Optimized File Name|File Name|Filename|SEO Name|SEO[- ]?Optimized)\s*:\s*(.*)', raw_text, re.IGNORECASE)
                if seo_match:
...
class DetailPopup(QDialog):
    ...
    def create_copyable_field(self, label_text, text_value, is_red=False):
        ...
        # Changed to QPlainTextEdit for exact 100% formatting preservation
        txt_box = QPlainTextEdit(text_value)
        txt_box.setReadOnly(True)
        txt_box.setFixedHeight(80 if label_text == t("prompt_lbl") else 60) 
        txt_box.setStyleSheet("background-color: #FFFFFF; border: 1px solid #D1D5DB; border-radius: 4px; color: #1F2937; font-size: 13px; font-family: Consolas, monospace;")
...
SEO Optimized File Name: [Your-File-Name]"""
    ...
    def auto_parse_input(self):
        ...
        if re.search(r'(Category|Cat|Topic)\s*:', text, re.IGNORECASE) and \
           re.search(r'(SEO[- ]?Optimized|File Name|Filename)\s*:', text, re.IGNORECASE):
            
            raw_blocks = re.split(r'(?=(?:Category|Cat|Topic)\s*:)', text, flags=re.IGNORECASE)
            entries_added = 0
            
            for block in raw_blocks:
                if not block.strip(): continue
                
                if re.search(r'(SEO[- ]?Optimized|File Name|Filename)\s*:', block, re.IGNORECASE):
                    try:
                        data = {}
                        cat_m = re.search(r'(?:Category|Cat|Topic)\s*:\s*(.*?)(?=(?:Aspect Ratio|AR|Ratio)\s*:)', block, re.IGNORECASE | re.DOTALL)
                        ar_m = re.search(r'(?:Aspect Ratio|AR|Ratio)\s*:\s*(.*?)(?=(?:AI Image Generation Prompt|Prompt|AI Prompt)\s*:)', block, re.IGNORECASE | re.DOTALL)
                        prompt_m = re.search(r'(?:AI Image Generation Prompt|Prompt|AI Prompt)\s*:\s*(.*?)(?=(?:Adobe Stock Title|Title|Headline)\s*:)', block, re.IGNORECASE | re.DOTALL)
                        title_m = re.search(r'(?:Adobe Stock Title|Title|Headline)\s*:\s*(.*?)(?=(?:Adobe Stock Keywords|Keywords|Tags)\s*:)', block, re.IGNORECASE | re.DOTALL)
                        kw_m = re.search(r'(?:Adobe Stock Keywords|Keywords|Tags)\s*:\s*(.*?)(?=(?:SEO[- ]?Optimized File Name|File Name|Filename|SEO Name|SEO[- ]?Optimized)\s*:)', block, re.IGNORECASE | re.DOTALL)
                        seo_m = re.search(r'(?:SEO[- ]?Optimized File Name|File Name|Filename|SEO Name|SEO[- ]?Optimized)\s*:\s*(.*)', block, re.IGNORECASE | re.DOTALL)
                        
                        data['category'] = cat_m.group(1).strip() if cat_m else ""
                        data['aspect_ratio'] = ar_m.group(1).strip() if ar_m else ""
                        data['prompt'] = prompt_m.group(1).strip() if prompt_m else ""
                        data['title'] = title_m.group(1).strip() if title_m else ""
                        data['keywords'] = kw_m.group(1).strip() if kw_m else ""
                        
                        raw_seo = seo_m.group(1).strip() if seo_m else ""
                        raw_seo = re.sub(r'\)$', '', raw_seo).strip()
                        data['seo_filename'] = re.sub(r'\.(jpe?g|png|webp|gif)$', '', raw_seo, flags=re.IGNORECASE)
                        
                        if data['category'] and data['seo_filename']:
    ...
    def upload_all_pending(self):
        reply = QMessageBox.question(self, 'Confirm Upload', "Are you sure you want to move all Pending assets to the 'Uploaded' list?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
            for item in self.data_list:
    ...
    def delete_all_pending(self):
        reply = QMessageBox.question(self, 'Confirm Delete', "Are you sure you want to delete ALL Pending assets? This action cannot be undone.", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
            self.bulk_delete_by_status('pending')

    def delete_all_uploaded(self):
        reply = QMessageBox.question(self, 'Confirm Delete', "Are you sure you want to delete ALL Uploaded assets? This action cannot be undone.", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.Yes:
    ...
    def export_csv(self):
        ...
        if export_count > 0:
            ...
            QMessageBox.warning(self, "No Assets Found", "No pending assets with images found to export.")
